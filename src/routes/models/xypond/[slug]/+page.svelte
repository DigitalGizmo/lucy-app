<script>
  ...
  import MomentNav from '../../../moments/components/MomentNav.svelte';
  import Moment from '../../../moments/components/Moment.svelte';
  export let data;
  ...
  $: if (currMomentIndex != scrolledXIndex) {
    ...
    // console.log('scrolledX ' + scrolledXIndex + ' currMomentIndex: ' + currMomentIndex);
    goto(`/models/xypond/${momentSlugs[currMomentIndex]}`)
  }
</script>
        ...
        <ul>
          {#if getPrevSlugIdx(moment.slug) >= 0 }
            <li class="prev-moment">
              <a href="/models/xypond/{momentSlugs[getPrevSlugIdx(moment.slug)]}"
          ...
          {#if getNextSlugIdx(moment.slug) <= 12 }
            <li class="next-moment">
              <a href="/models/xypond/{momentSlugs[getNextSlugIdx(moment.slug)]}"
